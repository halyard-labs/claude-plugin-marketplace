...
if grep -qE '"name":"[^"]*organisaton-kb__summarize_work"' "$TRANSCRIPT_PATH"; then
  exit 0
fi
...
{
  "decision": "block",
  "reason": "Optional, before finishing: if this session produced meaningful work or durable knowledge (code changes, decisions, debugging findings, research, or reusable answers from the user), capture it with mcp__plugin_halyard_organisaton-kb__summarize_work using the most appropriate entry_type (DECISION, PROCESS, or CONTEXT). If the session was trivial or there is nothing worth capturing, simply finish — no call is required."
}
EOF
