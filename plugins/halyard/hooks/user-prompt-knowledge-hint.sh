...
cat <<'EOF'
{
  "hookSpecificOutput": {
    ...
    "additionalContext": "Hint (ignore if not relevant): if this prompt contains durable, reusable knowledge — a decision, requirement, process, or context that would help a future session — it can be captured with mcp__plugin_halyard_org-kb__summarize_work, and mcp__plugin_halyard_org-kb__search_knowledge may surface prior work on the topic."
  }
}
EOF
